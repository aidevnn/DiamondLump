...
namespace NDarrayLib
{
    public abstract class Operations<U>
    {
        ...
        public abstract U Sq(U x);
        public abstract U Sqrt(U x);
        public abstract U Tanh(U x);
        public abstract U DTanh(U x);
        public abstract U Sigmoid(U x);
        public abstract U DSigmoid(U x);
    ...
    public class OpsInt : Operations<int>
    {
        ...
        public override int DTanh(int x) => throw new NotImplementedException();
        public override int Sigmoid(int x) => throw new NotImplementedException();
        public override int DSigmoid(int x) => throw new NotImplementedException();
        ...
        public override int Rand(int min, int max) => Utils.Random.Next(min, max);
    ...
    public class OpsFloat : Operations<float>
    {
        ...
        public override float DTanh(float x)
        {
            var x0 = (float)Math.Tanh(x);
            return 1f - x0 * x0;
        }
        public override float Sigmoid(float x) => (float)(1.0 / (1.0 + Math.Exp(-x)));
        public override float DSigmoid(float x)
        {
            var x0 = Sigmoid(x);
            return x0 * (1f - x0);
        }
        ...
        public override float Rand(float min, float max) => (float)(min + (max - min) * Utils.Random.NextDouble());
    ...
    public class OpsDouble : Operations<double>
    {
        ...
        public override double DTanh(double x)
        {
            var x0 = Math.Tanh(x);
            return 1.0 - x0 * x0;
        }
        public override double Sigmoid(double x) => 1.0 / (1.0 + Math.Exp(-x));
        public override double DSigmoid(double x)
        {
            var x0 = Sigmoid(x);
            return x0 * (1.0 - x0);
        }
        ...
        public override double Rand(double min, double max) => min + (max - min) * Utils.Random.NextDouble();
